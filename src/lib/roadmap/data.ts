...
export const ROADMAP_ITEMS: readonly RoadmapItem[] = [
  ...
  },
  {
    ...
    detail:
      'Retirement engine: accumulation phase, complete. Every simulated path used to start already retired at `currentAge`, with `retirementAge` carried but never read (see `docs/STATUS.md`\'s Phase 3 Milestone 3 section, "Genuine, plan-contradicting scope narrowing," for the original decision). Now: a person still short of their own `retirementAge` contributes (Phase 1\'s `pension_contribution` table, summed live in `resolveScenario.ts`) into `sipp_pension` instead of the household drawing down — household spending/withdrawal only starts once every alive person has reached their own retirement age. Two disclosed simplifications, not oversights: no partial-household drawdown (a working spouse\'s income isn\'t modelled as offsetting a retired partner\'s spending), and no relief-at-source grossing-up (contributions land exactly as entered; method-aware tax treatment is Phase 4.5\'s job). Surfaced in the UI, not just baked into the maths: the scenario editor\'s retirement-age field explains what it now does, and the results page shows a "Before retirement" note naming each not-yet-retired person\'s assumed contribution.',
    status: 'done',
