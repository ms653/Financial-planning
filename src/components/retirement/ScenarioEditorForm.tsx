...
/**
 * The Scenario Editor: three sections (When/Spending/Strategy) per
 * `docs/DESIGN_SPEC.md`, one primary action ("Run simulation" — saves the scenario and
 * starts a run in one sequence, see `useScenarioRunner.ts`'s own doc comment for why
 * there's no separate Save button), and a persistent results-preview strip.
 *
 * Client-side validation calls the real `validateScenarioForm` (which itself defers to
 * `parseScenarioAssumptions` for every format/range rule) on every render, the same
 * `AccountForm.tsx` pattern this codebase already uses — one set of rules, not two.
 */
...
export function ScenarioEditorForm({
  ...
  // Snapshots {name, isBaseline, values} as they were at the *last successful run* — not
  // the original `initial*` props, which never change again after mount. Comparing
  // against the fixed props would mean any scenario whose name was typed in (i.e. every
  // new scenario) reads as permanently dirty even the instant after a successful run,
  // since the current `name` would never again equal the empty `initialName` it started
  // from. Found by a real browser E2E run, not reasoned out in advance.
  function snapshotOf(n: string, baseline: boolean, v: ScenarioFormValues): string {
    return JSON.stringify({ name: n, isBaseline: baseline, values: v });
  }
  const [lastRunSnapshot, setLastRunSnapshot] = useState(() =>
    snapshotOf(initialName, initialIsBaseline, toFormValues(people, initialAssumptions)),
  );

  const validation = useMemo(() => validateScenarioForm(values), [values]);
  const liveErrors: FieldErrors = validation.ok ? {} : validation.errors;
  const dirty = snapshotOf(name, isBaseline, values) !== lastRunSnapshot;
  ...
  async function handleRunSimulation() {
    ...
    if (started) setLastRunSnapshot(snapshotOf(name, isBaseline, values));
  }
    ...
    <div className="space-y-7">
      ...
      <fieldset disabled={locked} className="space-y-7 disabled:opacity-60">
            ...
            {people.map((person, index) => (
              <div key={person.personId} className="grid gap-5 sm:grid-cols-3">
                <Field
                  ...
                  hint={`Currently ${person.currentAge}`}
        ...
        <section className="rounded-card border border-line bg-paper-sunken/50">
          ...
          {advancedOpen ? (
            <div className="space-y-6 border-t border-line px-5 pb-6 pt-5 sm:px-6">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-content-faint">
                  Applied literally, in this order — not an optimiser.
                </p>
      ...
      <div className="rounded-card border border-line bg-paper-sunken/40 p-4">
        {locked ? (
          <ComputingState
            iterationCount={runner.poll.run?.iterationCount ?? 2000}
          />
        ) : dirty ? (
