...
- **UK retirement accounts and rules differ substantially from the US model** — this is not a relabeling exercise:
  ...
  - **Tax band is a derived value, not an independent input — don't store it as a static field.** Pension contributions (relief-at-source or salary sacrifice) reduce *adjusted net income*, which is what actually determines someone's band, so a self-declared "I'm a higher-rate taxpayer" flag would silently go stale the moment contribution amounts change. Store **annual gross income per person** instead (an editable planning assumption, not precise payslip data), and derive band/allowance status from income minus pension contributions in the engine. This also surfaces the **personal allowance taper**: between £100,000 and £125,140 of adjusted net income, the personal allowance withdraws at £1 per £2 earned, creating an effective marginal rate around 60% in that band — and pension contributions are the standard lever to pull income back under £100k and reclaim it. That's a genuinely high-value recommendation for the Cash Allocation Advisor (Section 4) to be able to make, and it's only reachable if income, not a static band label, is the stored primitive.
...
- **Cash Allocation Advisor** — "where should my next £X go" recommendation, in two parts:
  - **Contribution waterfall** — a priority-ordered decision tree over the household's actual accounts: emergency fund topped up → pension contributions raised to at least the level needed to capture the **full employer match** (i.e. claim any portion of the match not currently being captured — an instant, effectively-guaranteed return, always first) → **if adjusted net income sits in the £100,000–£125,140 personal allowance taper zone, prioritize enough further pension contribution to pull income back under £100k** — this reclaims the personal allowance and addresses an effective ~60% marginal rate, generally the single highest-value move available before considering anything else below → high-interest debt (credit cards, personal loans — anything with a rate that beats realistic investment returns) cleared → LISA if eligible (25% government bonus) → remaining ISA allowance → further pension contributions (tax relief, **capped by the £60,000/year annual allowance** — lower if tapered for high earners or restricted to £10,000 by the Money Purchase Annual Allowance for anyone who's already flexibly accessed a pension; the waterfall must not recommend contributions past this cap) → GIA. LISA eligibility is two-tier and the engine needs both: you can only **open** one between ages 18–39, but once open you can keep **contributing** (and earning the 25% bonus) up to age 50 — an under-45 cutoff would be wrong on both counts. Tax band, personal-allowance-taper status, and annual-allowance taper are all **derived from each person's `annual_gross_income` minus pension contributions**, not from a static self-declared band (see Section 2) — this is what lets the advisor react correctly when contribution amounts change. Uses live account data (remaining ISA/LISA/pension annual allowance, current debt balances/rates) rather than generic advice.
...
household
 └─ person (1..n)
     └─ annual_gross_income (editable planning assumption — drives derived tax band, personal-allowance-taper status, and annual-allowance taper; not stored as a static band flag)
