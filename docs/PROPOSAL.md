...
- **Cash Allocation Advisor** — "where should my next £X go" recommendation, in two parts:
  ...
  - Needs new fields on the `debt` account type: interest rate, current balance, minimum payment, and (for mortgages specifically) Early Repayment Charge percentage limit, the charge's balance basis (original loan vs. current outstanding — lenders differ), and the charge period end date — not currently in the Phase 0/1 data model, add to Phase 1 scope.
...
household
 └─ person (1..n)
     └─ account (cash | gia | isa | lisa | sipp_pension | property | debt)
         ...
         └─ debt_terms (debt accounts only: interest_rate, current_balance, minimum_payment, mortgage_erc_limit_pct, mortgage_erc_balance_basis, mortgage_erc_period_end)  -- feeds the Cash Allocation Advisor
household
...
| 1 | Household/people/accounts (with tax-wrapper flag: ISA/LISA/pension/GIA/none, plus debt interest rate, balance, minimum payment, and mortgage ERC terms), manual net worth dashboard |
