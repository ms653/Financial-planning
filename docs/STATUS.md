...
24. ~~`next dev`'s env loading silently corrupts any `.env`/`.env.local` value containing `$` characters (found during Milestone 2)~~ **Fixed.** No dotenv-expand opt-out exists in this Next 14.2.35, and the two files can't share one escaping convention: `docker compose` passes `.env` values straight through unescaped, but backslash-escaping is the *only* shape `@next/env`'s bundled dotenv-expand leaves alone for `next dev`, so escaping `.env` would break Compose and leaving `.env.local` unescaped keeps breaking `next dev` — quoting protects against neither, contrary to the previous doc comment's claim. Fixed at the source instead of by hand-editing either file: `escapeForDotenvExpand` (`src/lib/auth/passphrase.ts`, unit-tested by actually round-tripping a hash through the real `@next/env` loader in a temp dir, not just asserting the regex) and `scripts/hash-passphrase.ts` now prints both a raw line (`.env`/Compose) and a pre-escaped line (`.env.local`/`next dev`), labelled by destination, so a hash is never hand-escaped or hand-copied between the two again. `.env.example` and `docs/DEPLOYMENT.md` updated to match and to warn against copying `.env`'s value straight into `.env.local`.
